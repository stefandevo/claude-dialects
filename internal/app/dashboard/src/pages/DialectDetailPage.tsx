      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card><CardHeader><CardTitle className="flex items-center gap-2"><Network className="size-4 text-primary" />Routing</CardTitle></CardHeader><CardContent><dl><DetailRow label="Preset" value={dialect.preset} />{dialect.presetDrift && <div className="flex flex-col gap-2 border-b py-3 last:border-b-0 sm:flex-row sm:items-start sm:justify-between sm:gap-6"><dt className="text-sm text-muted-foreground">Preset drift</dt><dd className="flex flex-col items-start gap-2 sm:items-end"><PresetDriftBadge drift={dialect.presetDrift} />{dialect.presetDrift.changes?.map((change) => <span key={change} className="text-xs text-muted-foreground">{change}</span>)}{dialect.presetDrift.command && <code className="rounded-md border bg-muted px-2 py-1 font-mono text-xs">{dialect.presetDrift.command}</code>}</dd></div>}<DetailRow label="Proxy port" value={dialect.port} mono /><DetailRow label="Bridge" value={dialect.bridge} /><DetailRow label="Bridge port" value={dialect.bridgePort} mono /><DetailRow label="Base URL" value={dialect.baseUrl} mono /></dl></CardContent></Card>
          </div>

          <Card><CardHeader><CardTitle className="flex items-center gap-2"><Braces className="size-4 text-primary" />Request behavior</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-2"><Badge variant="outline">Effort {dialect.effort ? 'enabled' : 'disabled'}</Badge><Badge variant="outline">Level {dialect.effortLevel || 'auto'}</Badge><Badge variant="outline">Concurrency {dialect.concurrency}</Badge><Badge variant="outline">Tool search {dialect.toolSearch ? 'enabled' : 'disabled'}</Badge>{dialect.contextWindow ? <Badge variant="outline">Context {dialect.contextWindow.toLocaleString('en-US')} tokens</Badge> : <Badge variant="destructive">Context window not set</Badge>}{dialect.authTokenEnv && <Badge variant="outline">Token via {dialect.authTokenEnv}</Badge>}{dialect.extraEnvKeys?.map((key) => <Badge key={key} variant="secondary">Env: {key}</Badge>)}</CardContent></Card>
        </div>
